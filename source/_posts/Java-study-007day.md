...
* [(C++) 참고용 정리 - 클래스의 두 가지 생성 방법](/2017/02/09/C-ref-005/)  
...
즉, `new` 연산자를 통해 클래스의 인스턴스가 생성되는 게 아니라!!  
주소값만을 얻어내는 연산자이지, 실제로는 생정자(TV())를 통해 만들어지게 되는 것이었다!!
...
객체 지향 언어인 Java에서는 문자열(데이터)와 문자열과 관련된 함수(함수)를 같이 모아  
...
```
우선 C언어의 static 변수는 스코프가 지연 변수의 성향을 띄는데 반해  
...
[(Java) 자바의 정석 3판 009일차 정리](http://localhost:4000/2017/02/11/Java-study-009day/#메소드-멤버-함수-클래스-메소드-amp-인스턴스-메소드-설계하기)
