...
책에서는 달러($)와 프랑(스위스 통화)의 연산에 대한 조그만 테스트를 시작으로 두 통화 사이의 중복을 제거해나갔다.  
해당 포스트도 위 두가지 통화에 대해 간단한 연산을 테스트 하는 작은 코드로 시작한다. 
...
class Dollar(amount: Int) {
    operator fun times(multiplier: Int): Any {
...
2. times 메서드의 반환 타입을 Any로 정했다.
3. times 메서드 내부를 TODO로 비워뒀다.
...
우선 하나의 테스트만 가지고는 제대로 구현했는지 검증이 안 되므로 not equals에 대한 테스트도 작성해보자.
```kotlin
@Test
fun `test not equality`() {
    ...
    // Then
    val notExpected = Dollar(7)
    actual shouldNotBe notExpected
}
```
Dollar 클래스의 equals 메서드는 무조건 true를 반환하므로 테스트는 성공하지 못한다.
equality를 비교하려면 해당 클래스 안에 있는 값을 검사하는 값 객체(Value Object) 패턴을 사용해야한다.
...
테스트를 돌려보면 test equality는 통과하는데 test multiplication은 times 메서드를 제대로 구현하지 않았기 때문에 실패한다.
이제 test multiplication 메서드를 통과시키도록 times 메서드의 구현부를 바꿔보자.
...
$6 * 3 = $18 테스트에서 실패한다.
`org.opentest4j.AssertionFailedError: expected:<18> but was:<10>`

times 메서드의 반환값을 Dollar(`10`)이라는 상수를 박았기 때문에 실패했다.  
따라서 10이라는 상수를 변수로 변환시켜보면 아래와 같이 **중복을 제거**할 수 있다.
```kotlin
...
우선 상수를 사용하던 코드로 돌아가서 Dollar(`10`) 부분을 좀 더 집중해서 보면 우리 머릿 속에서 엄청 빠른 연산이 일어나서 10이란 값이 나온 거지 사실은 연산을 거치기 전에는 아래와 같다.
```kotlin
operator fun times(multiplier: Int): Dollar {
    return Dollar(5 * 2)
}
```
바로 테스트의 `val expected = Dollar(amount * multiplier).amount` 부분(**amount * multiplier**)과 중복되는 걸 볼 수 있다.  
따라서 우리는 Dollar 클래스의 **amount** 필드와 times 메서드의 매개변수인 **multiplier**를 사용하여 중복을 제거했다.  
...
class FrancTest {
    ...
    fun `test multiplication`() = listOf(
        ...
        dynamicTest("$${amount} * $multiplier = $${amount * multiplier}") {
            // Given
            val francs = Franc(amount)

            // When
            val actual = francs * multiplier

            // Then
            val expected = Franc(amount * multiplier)
            actual shouldBe expected
        }
    }
    ...
    fun `test not equality`() {
        ...
        // Then
        val notExpected = Franc(7)
        actual shouldNotBe notExpected
    }
}
```

이제 컴파일이 되도록 하면 되는데 우리는 어떻게 구현해야할지 Dollar 클래스를 테스트하면서 명확해졌으므로 Dollar 클래스의 구현체도 복붙하자.
```kotlin
...
모든 부분을 Dollar 테스트를 통해 검증했으므로 딱히 손 볼 곳이 없다.

## Dollar/Franc 중복 제거하기
우선 두 클래스의 times/equals 메서드를 보면 반환 타입만 다르거나 타입 캐스팅하는 부분만 다를 뿐, 로직은 동일하다.  
이 로직의 중복을 제거하려면 어떻게 해야할까?  
가장 빠르게 떠오른 방법은 두 클래스의 중복을 묶어줄(?) 상위 클래스가 있으면 될 거 같다.  

우선 DollorTest에서 Money 타입을 사용하도록 아래와 같이 수정해보자.
```kotlin
class DollarTest {
    @TestFactory
    fun `test multiplication`() = listOf(
        ...
        dynamicTest("$${amount} * $multiplier = $${amount * multiplier}") {
            ...
            // When
            val actual: Money = dollars * multiplier
...
우선 테스트가 통과하긴 하는데... 이렇게 해선 times/equals 메서드를 Dollar 클래스에서 제거할 수 없다. (Franc 클래스에서도 마찬가지일 것이다.)  
인터페이스의 times 메서드를 디폴트 메서드로 빼면 되긴 하는데 개인적으로 인터페이스의 취지에 적합하지 않다고 판단하여 적당히 타협하여 인터페이스를 추상 클래스로 변경해보자.
```kotlin
...
오! 모든 테스트가 통과하고 드디어 Dollar 클래스에서 times/equals 메서드를 제거했고, Dollar 클래스만 봤을 때 딱히 하는 일이 없어 보인다.  
뭔가 냄새...가 나지만 아직은 좀 참고, Franc 메서드의 times/equals도 제거해보자.  
...
class FrancTest {
    ...
    fun `test multiplication`() = listOf(
        ...
        dynamicTest("$${amount} * $multiplier = $${amount * multiplier}") {
            // Given
            val dollars: Money = Franc(amount)
...
오! 드디어 Franc에서도 times/equals 메서드를 제거했다.
하지만 테스트를 돌려보면 test multiplication 테스트가 깨진다.
`org.opentest4j.AssertionFailedError: expected:<com.example.demo.Franc@3e08ff24> but was:<com.example.demo.Dollar@70ed52de>`

Money 클래스의 times 메서드가 Dollar 구체 클래스를 반환하기 때문에 발생하는 문제다.
...
DollarTest 클래스에서 Money의 하위 클래스인 Dollar 클래스에 직접적으로 접근하는 부분은 생성자를 통해 생성하는 부분과 times 메서드의 반환타입이다.
생성자는 팩토리 메서드를 통해 직접 참조를 제거하면 된다.
```kotlin
class DollarTest {
    @TestFactory
    fun `test multiplication`() = listOf(
        ...
        dynamicTest("$${amount} * $multiplier = $${amount * multiplier}") {
            // Given
            val dollars: Money = Money.dollar(amount)

            // When
            val actual: Money = dollars * multiplier

            // Then
            val expected: Money = Money.dollar(amount * multiplier)
            actual shouldBe expected
        }
    }
    ...
    fun `test not equality`() {
        ...
        // Then
        val notExpected: Money = Money.dollar(7)
        actual shouldNotBe notExpected
    }
}
```
...
abstract class Money {
    ...
    // 팩토리 메서드가 부모 클래스 타입을 리턴하도록 변경하였으므로 부모 클래스 API에 times 메서드가 추가돼야한다.
    abstract operator fun times(amount: Int): Money
}
```
Money에 Dollar 객체를 생성하는 static factory method를 추가했고 times 추상 메서드도 추가했다.
...
class FrancTest {
    ...
    fun `test multiplication`() = listOf(
        ...
        dynamicTest("$${amount} * $multiplier = $${amount * multiplier}") {
            // Given
            val francs: Money = Money.franc(amount)

            // When
            val actual: Money = francs * multiplier

            // Then
            val expected: Money = Money.franc(amount * multiplier)
            actual shouldBe expected
        }
    }
    ...
    fun `test not equality`() {
        ...
        // Then
        val notExpected: Money = Money.franc(7)
        actual shouldNotBe notExpected
    }
}
```
...
이제 모든 테스트가 통과한다.
외부에서 간단하게 바라봤을 때는 객체의 생성이 **직접적인 생성자를 통한 생성**에서 **외부 클래스의 static factory method를 통한 생성**으로 바뀌었고, times 메서드의 **반환 타입이 부모 클래스**로 바뀌었을 뿐이다.
...
여기서 멈추지 말고 아까 제거에 성공했던 equals 메서드도 부모 클래스로 올려서 하위 클래스에서 제거해보자.  
...
## 마치며
times 메서드를 제거하지 못한 건 아쉽지만, times 메서드를 제거하려면 통화를 다뤄야하므로 내용이 훨씬 길어질 것 같아서 여기서 잘랐다.
