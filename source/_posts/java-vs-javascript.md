...
![사용자가 메뉴에 마우스를 올리면 하위 메뉴가 보이는 드롭다운 메뉴](drop.png)  
![사용자가 입력한 값을 얻어와서 올바른 값을 입력했는지 현재 페이지에서 유효성 검사하기](valid.png)
...
## 자바의 탄생 배경
### Oak
가전제품에 탑재할 소프트웨어를 만들 목적으로 썬마이크로 시스템즈에서는 Oak(오크)라는 프로그래밍 언어를 만들었다.
### 자바
WWW의 출현과 더불어 웹의 폭발적 성장으로 인해 Oak의 개발 방향을 인터넷에 맞게 돌려서 개발한 것이 Java라고 알고 있다.  
따라서 백엔드, 서버 쪽 진영을 보면 그렇게 자바를 많이 쓰는 이유가 자바가 **웹에 쓸 목적**으로 만든 언어이기 때문이다.  
