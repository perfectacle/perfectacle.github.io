...
---
![출처: http://www.datamodel.com/index.php/2014/06/24/big-data-nosql-and-data-modeling-big-challenges-in-data-modeling/](thumb.png)
...
```
응집도가 일반가/할인가를 포함하는 Period라는 테이블을 따로 파서 엔티티로 만들고,
외래키를 사용하여 Deal 테이블과 매핑하였다.
...
```
외래키가 deal 테이블의 id로 잡아서 각각 period, price 테이블에 뒀다고 가정한다.  
하지만 테이블을 일일이 쪼개기란 매우 귀찮은 일이다.  
...
EmbeddedType을 정의할 클래스네 @Embeddable 어노테이션을 달아줘야하고,  
...
```
필드를 일일이 나열하는 대신 응집도가 높게 EmbeddedType으로 정의해놓고,
...
따라서 변화를 감지해야하는 경우에는 엔티티로 만들어야할 것이다.  
