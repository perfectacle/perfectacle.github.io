...
![출처: http://kryptophone.kryptotel.net/faq/encryption/index.html](thumb.png)
...
`3번 과정에서 제 3자가 개입해서 만든 공개키인 경우에 제 3자에게 데이터를 전달하고, 이 데이터를 제3자가 본인의 개인키로 복호화가 가능하다.`  
위 취약점대로라면 내가 원치 않는 사람에게 데이터가 탈취됐다고 보면 된다.

#### 전자 인증서
![해당 블로그의 전자 인증서](certificate.png)  
...
### 전자 서명(비밀키 암호화)
비밀키를 통해 암호화하는 경우를 말한다.  
비밀키로 암호화 했다는 것은 공개키로만 풀 수 있다는 뜻이다.  
공개키로만 암호화 했다니 아래와 같은 의문을 품을 수 있다.  
