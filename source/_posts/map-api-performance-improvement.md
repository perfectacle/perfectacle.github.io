...
![](/images/map-api-performance-improvement/thumb.png)  

## 문제점
![크롬 개발자 도구 네트워크 탭에서 본 API 호출](/images/map-api-performance-improvement/01.png)  
Time은 데이터 전체를 파싱하는데 걸린 시간이니 무시하고...
트래픽이 13MB 남짓...  
사용자가 조건을 바꿔서 검색을 한다면 데이터 광탈범이 될 가능성이 다분한 상황이었다.  

![순수 응답 시간을 알고 싶어서 다운로드 받아봄](/images/map-api-performance-improvement/03.png)  
응답 시간이 22초 남짓...  

![StopWatch를 통해 어느 작업에서 병목이 가장 많이 발생하는지 파악](/images/map-api-performance-improvement/16.png)  

## 원인 파악
### 쿼리
![숙박 연동사 실제 가격 주입하는 부분](/images/map-api-performance-improvement/17.png)  
숙박 연동 최저가는 jooq로 불러오고 있고, 숙박 연동사 테이블은 jpa로 불러오고 있음.  
순수 네이티브 쿼리가 아닌 이상 퍼포먼스가 제대로 나오지 않을 것으로 판단됨.  

![딜 목록 불러오는 쿼리](/images/map-api-performance-improvement/05.png)  
실제로 필요한 건 특정 필드 뿐인데 모든 필드를 다 긁어오고 있어서 쿼리 실행속도가 느려진 것임.  

### 용량
![실제로 저장된 응답값](/images/map-api-performance-improvement/08.png)  
이 응답값에는 세 가지 문제점이 존재한다.

![쓸 데 없는 공백을 포함하고 있었다.](/images/map-api-performance-improvement/09.png)  
이 데이터를 줄였을 때 1MB 정도 가량이 줄어들었다.

![쓸 데 없는 컬럼들도 포함하고 있었다.](/images/map-api-performance-improvement/10.png)  
treeAllId라던지, clusterName이나 빈 배열 등등 다른 값들을 가지고 유추할 수 있는 값들을 제거하였다.
딱히 이 부분에서는 데이터를 크게 줄일 수가 없었다.  
  
![컬럼의 값을 가공하지 않고 그대로 들고 있다.](/images/map-api-performance-improvement/11.png)  
...
![이 화면에서 클러스터를 그리기 위한 정보는 중심 좌표, 지역 코드, 딜 갯수가 끝이다.](/images/map-api-performance-improvement/12.png)  
...
### 중간 점검
![데이터는 2MB 남짓으로 줄어들었고, 응답속도도 11초 가량 걸렸다.](/images/map-api-performance-improvement/18.png)
  
![가장 오래 걸리는 게 숙박 딜 실제 가격 주입 부분이다.](/images/map-api-performance-improvement/19.png)  
...
### 또 다시 중간 점검
![4~6초 가량으로 줄어들었다.](/images/map-api-performance-improvement/20.png)  

![최대 오래 걸리는 게 딜 목록을 불러오는 부분이다.](/images/map-api-performance-improvement/21.png)  

![제일 데이터가 큰 컬럼인 LIST_IMAGE_JSON 컬럼을 빼자 속도가 3배 가량 빨라졌다.](/images/map-api-performance-improvement/26.png)
...
![훨씬 빨라진 실행 속도들](/images/map-api-performance-improvement/27.png)

하지만 여전히 2~4초 가량 걸리는 상황이었고, 숙박 연동사의 가격을 주입하는 부분은 쿼리문을 쓰지도 않는데 왜 이렇게 오래 걸리는지 한번 파보았다.  
![수많은 리스트 사이에서 딜 아이디 가지고 원하는 요소를 찾는 구문](/images/map-api-performance-improvement/30.png)  
...
![총 실행 시간이 2초 내외로 줄어들었다.](/images/map-api-performance-improvement/31.png)  

![클러스터 목록을 불러오는 쿼리문](/images/map-api-performance-improvement/32.png)
...
![](/images/map-api-performance-improvement/33.png) 

## 차후 개선 사항 (시간 문제 및 공수와 효율성 문제)
* 2MB로 줄였다 하더라도 필터를 계속해서 바꾸다 보면 유저 입장에서는 부담되는 용량일 수도 있다.  
![딜을 내려주는 API에서 반복되는 키값을 빼고 순서를 보장한 배열로 만들어 내려주는 형태로 바꿔주면 데이터를 0.5MB 이상 단축할 수 있다.](/images/map-api-performance-improvement/15.png)    
