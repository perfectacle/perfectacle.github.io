...
![좌측 상단에 있는 + 버튼을 눌러서 Gradle을 선택하자.](spring-boot-2-env/add-gradle-task.png)  
![default로 설정한 profile이 local이기 때문에 딱히 profile argument를 넘겨주지 않아도 된다.](spring-boot-2-env/boot-run-local.png)  
