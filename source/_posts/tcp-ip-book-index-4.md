...
![윈도우의 CMD에서 ipconfig를 쳐보면 익숙한 서브넷 마스크가 보인다.](subnet-mask-windows.gif)
...
3. 호스트와 공유기는 같은 네트워크에 속하므로 프라이빗 ip로 통신하고, 대부분 공유기(라우터)의 ip는 호스트 부분이 1인 경우가 많으니 192.168.1.1로 데이터를 보낸다.  
...
네트워크에 새로 연결된 장비가 라우터(공유기)를 찾기 위해 9번 타입으로 브로드캐스팅하고,  
...
근거리 통신(LAN)에는 서브넷 마스크가 255.255.255.0인 네트워크를 통해 통신이 되고,  
WAN의 경우에는 255.255.255.128인 네트워크를 타고 통신이 된다.  
...
http://d2.naver.com./helloworld/238638으로 접속하면 com.뒤에 뭔가 올 거 같은데 입력하지 않고 바로 저렇게 입력해도 잘 접속이 된다.  
...
![naver를 추적해봤다.](traceroute-naver.png)  
