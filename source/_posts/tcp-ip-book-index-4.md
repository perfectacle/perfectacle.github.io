...
![](/images/tcp-ip-book-index-4/thumb.png)  
...
![AWS의 서비스 중 하나인 VPC](/images/tcp-ip-book-index-4/vpc.png)  
...
![윈도우의 CMD에서 ipconfig를 쳐보면 익숙한 서브넷 마스크가 보인다.](/images/tcp-ip-book-index-4/subnet-mask-windows.gif)
![맥에서는 환경설정 - 네트워크 - 본인이 연결한 이더넷 - Advanced를 누르고 TCP/IP 탭으로 오면 서브넷 마스크를 볼 수 있다.](/images/tcp-ip-book-index-4/subnet-mask-mac.png)  
...
![AWS에서는 VPC를 다시 서브넷으로 나눌 수 있다.](/images/tcp-ip-book-index-4/subnet.png)  
...
![AWS는 돈이 많아서인지 A 클래스의 프라이빗 어드레스와 B 클래스의 프라이빗 어드레스를 모두 소유하고 있다.](/images/tcp-ip-book-index-4/vpc2.png)  
![우리집에서 private ip 주소를 보니 내 프라이빗 IP는 C 클래스의 IP이다.](/images/tcp-ip-book-index-4/private-ip.png)  
...
![AWS에서 라우팅 테이블은 특정 VPC에 종속적이다.](/images/tcp-ip-book-index-4/routing-table.png)  
...
![AWS에서도 NAT를 지원해준다.](/images/tcp-ip-book-index-4/nat-gateway.png)  
...
![윈도우에서는 CMD에서 ipconfig를 치면 본인이 연결한 이더넷에 default gateway의 주소가 라우터(공유기)의 IP 주소이다.](/images/tcp-ip-book-index-4/router-windows.jpg)
![맥에서는 환경설정 - 네트워크 - 본인이 연결한 이더넷 - Advanced를 누르고 TCP/IP 탭으로 오면 라우터(공유기)의 IP 주소를 볼 수 있다.](/images/tcp-ip-book-index-4/router-mac.png)
...
![라우터에서 본 서브넷 마스크](/images/tcp-ip-book-index-4/subnet-mask-router.png)  
직접 라우터의 IP 주소와 네트워크의 서브넷 마스크도 조절이 가능한 것 같은데 뭔가 무서워서 함부로 건들지 못하겠다 ㅋㅋ
또한 이더넷 카드의 Physical Address인 Mac Address도 있다.

![라우터에서 볼 수 있는 다양한 정보들](/images/tcp-ip-book-index-4/router-other-info.png)  
...
![라우터에서 IP 할당에 관여하는 DHCP 설정도 볼 수 있다.](/images/tcp-ip-book-index-4/router-dhcp.png)  
IP 주소 대역을 보아하니 호스트는 100~199까지 총 100대만 연결이 가능한 네트워크인 모양이다.  
또한 임대 시간이 나와있는데 해당 시간을 초과한다고 해서 무조건적으로 IP 주소가 바뀌는 건 아닐 것이다.  

![라우터에서 인터넷 계층에 쓰이는 세그먼트의 사이즈인 MTU도 설정할 수 있다.](/images/tcp-ip-book-index-4/router-mtu.png)  
웬만하면 기본값인 1500바이트를 건들 일은 없어 보인다.

![정적으로 라우팅 테이블 설계도 가능하다.](/images/tcp-ip-book-index-4/router-routing-table.png)
...
![공유기 제조사마다 다른데 대부분은 포트포워딩 이라고 표현하고 있다.](/images/tcp-ip-book-index-4/router-port-forwarding.png)  
만약 배틀넷 서버가 클라이언트(192.168.0.181)에게 8123 포트로 요청을 보낸다고 하면  
위와 같이 등록을 해두면 된다.  

![웹 서비스를 포트포워딩 하는 경우](/images/tcp-ip-book-index-4/router-port-forwarding2.png)  
...
![IPv4와 IPv6의 루프백 어드레스로 127.0.0.1이 등록돼있고, 255.255.255.255로 브로드캐스팅 주소가 등록돼있다.](/images/tcp-ip-book-index-4/hosts.png)  
...
![AWS의 Route53에서 특정 도메인에 대해 등록된 리소스 레코드](/images/tcp-ip-book-index-4/route-53.png)  

#### 4-8-7. 리소스 레코드의 의미
각각의 레코드에 대해서는 나도 아직 공부하는 중이므로 아래 링크를 참조하자 ㅠㅠ  
[DNS Zone 파일에 쓰이는 레코드 설명](http://ricky840.tistory.com/10)  

![GoDaddy에 등록된 내 도메인에 대한 리소스 레코드](/images/tcp-ip-book-index-4/go-daddy.png)
...
백문이 불여일견 그냥 샘플을 봐보자.  
![내 블로그 주소를 추적해봤다.](/images/tcp-ip-book-index-4/traceroute-blog.png)  
...
![naver를 추적해봤다.](/images/tcp-ip-book-index-4/traceroute-naver.png)  
...
![www.naver.com.nheos.com으로 접속하면 www.naver.com으로 바뀌어서 접속이 된다.](/images/tcp-ip-book-index-4/naver-domain.png)  
